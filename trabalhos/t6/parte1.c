...
{
  int myrank, nproc, from, to, count, nrows, i, j, k;
  int tag_A = 0;
  int tag_B = 1;
  MPI_Status status;
  ...
  int A2[nrows][SIZE];
  MPI_Scatter(A, count, MPI_INT, A2, count, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Bcast(B, SIZE*SIZE, MPI_INT, 0, MPI_COMM_WORLD);

  printf("computing slice %d (from row %d to %d)\n", myrank, from, to-1);
  for (i=0; i<nrows; i++) {
    for (j=0; j<SIZE; j++) {
      C[myrank*nrows+i][j]=0;
      for (k=0; k<SIZE; k++){
        C[myrank*nrows+i][j] += A2[i][k]*B[k][j];
      }
    }
  }
